...
public class AckConsumer {
    ...
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(name = RabbitMqConstant.TOPIC_ACK_ACK_QUEUE, durable = "true", arguments = {
                    ...
                    @Argument(name = "x-dead-letter-exchange", value = RabbitMqConstant.DEAD_EXCHANGE), // 死信队列交换机-dead-exchange
                    @Argument(name = "x-dead-letter-routing-key", value = "xxx")}), // 死信队列路由key
            exchange = @Exchange(name = RabbitMqConstant.TOPIC_ACK_EXCHANGE, type = ExchangeTypes.TOPIC),// 交换机
            key = {"#"}// 路由key,通配符
    ))
